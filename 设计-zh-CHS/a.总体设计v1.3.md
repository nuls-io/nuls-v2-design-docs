...
靠一个团队来做好区块链操作系统的搭建、区块链技术的创新、企业级应用的支撑是非常困难的。我们要整合全世界所有社区伙伴的力量，需要一个更开放、包容的系统架构，降低社区社区成员参与的门槛，让所有想参与的成员都可以参与到nuls的贡献中，完善nuls，指引nuls走向更好的明天。
...
2、tx_a在A链中打包，并在n块确认后，被跨链模块（独立于A链功能之外的基本模块）发现该交易，并将此交易根据跨链协议，生成一个卫星链的交易tx_a_trans。

3、跨链模块将该交易广播给连接的卫星链节点（广播方式：先广播hash，等待对方获取完整交易）

4、卫星链节点（交易管理模块）收到该交易（tx_a_trans）后，先进行基础校验（格式、必填字段、链余额等），通过后询问连接的A链节点，是否有一个tx_a_hash的交易被确认了n块，且转换为卫星链格式交易后，交易摘要为tx_a_trans_hash。
